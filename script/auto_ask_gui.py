...
class ProcessingWorker(QObject):
    """
    A worker object that runs the processing task in a separate thread.
    """
    progress_update = pyqtSignal(str)
    processing_finished = pyqtSignal(str)
    processing_error = pyqtSignal(str, str)  # title, message
    ...
    def run(self):
        ...
        try:
            ...
            prompt_template = self.params['prompt_template']
            output_template = self.params['output_template']
            api_name = self.params['api_name']
            use_openai = api_name == 'OpenAI'

            if not os.path.exists(input_file):
                raise FileNotFoundError(f'输入文件不存在: {input_file}')
            if not os.path.exists(os.path.dirname(output_file)):
                raise FileNotFoundError(f'输出目录不存在: {os.path.dirname(output_file)}')
            ...
            if separator:
                try:
                    ...
                    raise ValueError(f'正则表达式格式错误：{str(e)}')
            else:
                segments = content.splitlines()

            with open(output_file, 'w', encoding='utf-8') as f:
                segment_count = len(segments)
                logger.info(f'共 {segment_count} 个片段待处理。')
                ...
                for i, segment in enumerate(segments):
                    ...
                    logger.info(f'正在处理片段 {i + 1}/{segment_count}...')
                    prompt = prompt_template.replace('{original_text}', segment)

                    response = chat_with_openai(prompt, model, server_url) if use_openai else chat_with_ollama(prompt, model,
                                                                                                           server_url)

                    if response:
                        result = output_template.replace('{original_text}', segment).replace('{output_text}', response)
                        f.write(result + '\n')
                        self.progress_update.emit(f'处理完成: {segment[:50]}...')
                        logger.info(f'片段处理成功: {segment[:50]}...')
                    else:
                        self.processing_error.emit(f'{api_name}连接错误',
                                                   f'无法连接到{api_name}服务器({server_url})，请检查服务是否运行且地址正确。')
                        return

            if not self._is_stopped:
                self.processing_finished.emit('所有处理完成！')

        except Exception as e:
            error_msg = str(e)
            logger.error(f'处理过程中发生未知错误: {error_msg}', exc_info=True)
            self.processing_error.emit('错误', f'处理过程中发生错误：\n{error_msg}')
...
class AutoAskGUI(QMainWindow):
    ...
    def on_processing_error(self, title, message):
        logger.error(f'处理错误: {title} - {message}')
        self.status_edit.append(f'错误: {message}')
    ...
    def closeEvent(self, event):
        ...
        if self.processing_thread and self.processing_thread.isRunning():
            ...
            if not self.processing_thread.wait(3000):  # Wait 3s for graceful exit
                logger.warning("线程未能正常停止，将强制终止。")
    ...
    def start_processing(self):
        ...
        params = {
            ...
            'api_name': 'OpenAI' if self.openai_radio.isChecked() else 'Ollama'
        }

        if not all([params['input_file'], params['output_file'], params['prompt_template'], params['output_template']]):
        ...
        self.status_edit.append('正在准备处理...')
        ...
        self.worker.processing_error.connect(self.processing_thread.quit)
        self.processing_thread.finished.connect(self.processing_thread.deleteLater)
        self.worker.processing_finished.connect(self.worker.deleteLater)
        self.worker.processing_error.connect(self.worker.deleteLater)

        self.processing_thread.start()
        self.status_edit.append('处理任务已启动...')
